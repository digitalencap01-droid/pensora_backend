...
class ContentProject(TimestampMixin, Base):
    ...
    webflow_item_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
...
class ResearchRun(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "project_id", "version", name="uq_research_runs_project_version"
        ),
    )
...
class KeywordStrategyRecord(TimestampMixin, Base):
    __tablename__ = "keyword_strategies"

    __table_args__ = (
        UniqueConstraint(
            "project_id", "version", name="uq_keyword_strategies_project_version"
        ),
    )
    ...
    search_intent: Mapped[str] = mapped_column(String(100), nullable=False)
    payload: Mapped[dict] = mapped_column(JSON, nullable=False)


class ContentBriefRecord(TimestampMixin, Base):
    __tablename__ = "content_briefs"

    __table_args__ = (
        UniqueConstraint(
            "project_id", "version", name="uq_content_briefs_project_version"
        ),
    )
...
class ArticleRecord(TimestampMixin, Base):
...
class ArticleVersionRecord(TimestampMixin, Base):
    ...
    article_markdown: Mapped[str] = mapped_column(Text, nullable=False)
    payload: Mapped[dict] = mapped_column(JSON, nullable=False)


class SEOMetadataRecord(TimestampMixin, Base):
    __tablename__ = "seo_metadata"

    __table_args__ = (
        UniqueConstraint(
            "project_id", "version", name="uq_seo_metadata_project_version"
        ),
    )
...
class GeneratedFileRecord(TimestampMixin, Base):
...
class DocumentChunkRecord(TimestampMixin, Base):
...
class DocumentImageRecord(TimestampMixin, Base):
...
class ImageBatch(TimestampMixin, Base):
...
class LinkedInConnection(TimestampMixin, Base):
    ...
    linkedin_member_id: Mapped[str] = mapped_column(String(200), nullable=False)
    linkedin_name: Mapped[str | None] = mapped_column(String(300), nullable=True)
    linkedin_email: Mapped[str | None] = mapped_column(String(300), nullable=True)
...
class WebflowConfig(TimestampMixin, Base):
    ...
    title_field: Mapped[str] = mapped_column(
        String(100), nullable=False, default="name"
    )
    slug_field: Mapped[str] = mapped_column(
        String(100), nullable=False, default="slug"
    )
    body_field: Mapped[str] = mapped_column(
        String(100), nullable=False, default="post-body"
    )
    summary_field: Mapped[str] = mapped_column(
        String(100), nullable=False, default="post-summary"
    )
    main_image_field: Mapped[str | None] = mapped_column(String(100), nullable=True)
    thumbnail_field: Mapped[str | None] = mapped_column(String(100), nullable=True)


class UploadedImageRecord(TimestampMixin, Base):
    __tablename__ = "uploaded_images"

    __table_args__ = (
        UniqueConstraint(
            "batch_id", "order_index", name="uq_uploaded_images_batch_order_index"
        ),
    )
    ...
    batch_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("image_batches.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    order_index: Mapped[int] = mapped_column(Integer, nullable=False)
